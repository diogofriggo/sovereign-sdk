...
use sov_eigenda_adapter::spec::{EigenDaSpec, RollupParams};
...
use crate::{eth_dev_signer, EIGENDA_CERT_RECENCY_WINDOW, EIGENDA_ROLLUP_BATCH_NAMESPACE, EIGENDA_ROLLUP_PROOF_NAMESPACE};

/// Rollup with EigenDa
#[derive(Default)]
pub struct EigenDaNomtDemoRollup<M> {
    phantom: std::marker::PhantomData<M>,
}

type Hasher = <Risc0CryptoSpec as CryptoSpec>::Hasher;
type NativeStorage =
    NomtProverStorage<DefaultStorageSpec<Hasher>, <EigenDaSpec as DaSpec>::SlotHash>;

type EigenDaRollupSpec<M> = ConfigurableSpec<
    EigenDaSpec,
    Risc0,
    MockZkvm,
    MultiAddressEvm,
...
impl RollupBlueprint<Native> for EigenDaNomtDemoRollup<Native>
where
    EigenDaRollupSpec<Native>: PluggableSpec,
    <EigenDaRollupSpec<Native> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = EigenDaRollupSpec<Native>;
    type Runtime = Runtime<Self::Spec>;
}

impl RollupBlueprint<WitnessGeneration> for EigenDaNomtDemoRollup<WitnessGeneration>
where
    EigenDaRollupSpec<WitnessGeneration>: PluggableSpec,
    <EigenDaRollupSpec<WitnessGeneration> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = EigenDaRollupSpec<Native>;
    type Runtime = Runtime<Self::Spec>;
}
...
#[async_trait]
impl FullNodeBlueprint<Native> for EigenDaNomtDemoRollup<Native> {
    type DaService = EigenDaService;

    type StorageManager = NomtStorageManager<EigenDaSpec, Hasher, NativeStorage>;
    ...
    async fn create_da_service(
        ...
        .await
        .expect("EigenDaService configuration is valid")
    }
...
impl WalletBlueprint<Native> for EigenDaNomtDemoRollup<Native> {}
